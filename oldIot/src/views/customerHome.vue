<template>
  <el-container class="customerHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
...
export default {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: '/customerMain'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/customerMain' && this.$route.path !== '/customerHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/customerMain', name: '护士首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/customerMain', name: '护士首页'})
      this.$store.commit('set_active_index', '/customerMain')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/customerHome' && to.name === undefined) {
          return
        }
